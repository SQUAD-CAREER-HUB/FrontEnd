...
export default function InterViewStage() {
  ...
  return (
    <StageWrapper number={3} stage="INTERVIEW">
      <div className={`rounded-xl p-5 shadow-sm transition-all border bg-white dark:bg-slate-900 ${activeStage === 'INTERVIEW' ? ACTIVE_STAGE_STYLES.border : 'border-slate-200 dark:border-slate-700'}`}>
        <div className='flex justify-between items-center mb-5'>
          <h3 className={`font-bold text-lg ${activeStage === 'INTERVIEW' ? 'text-brand-600 dark:text-brand-400' : STATUS_FONT_STYLES[result]}`}>
            면접 전형
          </h3>
          <button
