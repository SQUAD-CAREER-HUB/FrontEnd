import { Clock, Pen, Trash2 } from "lucide-react";
...
export default function EtcViewCard({
  id,
  title,
  datetime,
  endDatetime,
  ...
  const { setEditingStageId } = useStageEditor(id, 'other');
  const { mutate: deleteSchedule } = useDeleteSchedule('other');
          ...
          <div className="flex-1 min-w-0 pr-4">
            ...
            <div className="text-[11px] text-slate-500 mt-1.5 flex flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-slate-400 mr-0.5">시작:</span>
                {formatDateTime(datetime)}
              </div>
              {endDatetime && (
                <div className="flex items-center gap-1.5">
                  <Clock className="w-3 h-3 text-slate-400" />
                  <span className="text-slate-400 mr-0.5">종료:</span>
